...
const Order = () => {
  const [orders, setOrders] = useState([]);
  const [user, setUser] = useState(null); // 🟡 Track user for auth check
  ...
  const fetchUser = async () => {
    try {
      ...
      if (res.data.success) {
        setUser(res.data.data); // set user if authenticated
        fetchAllOrders(); // ✅ only fetch orders if user exists
      } else {
  ...
  useEffect(() => {
    fetchUser(); // 🟡 start by checking login status
  }, []);
